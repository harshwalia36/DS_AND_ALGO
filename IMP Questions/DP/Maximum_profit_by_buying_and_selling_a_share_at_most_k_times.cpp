...
//  profit[t][i] = max(profit[t][i-1], max(price[i] – price[j] + profit[t-1][j])) 
        //   for all j in range [0, i-1]               --------> O(k*N^2) but can be optimized to O(k*n)
...
int maxProfit(int price[], int n, int k)
{
    ...
    {   int prevDiff=INT_MIN;
        for(int j=1;j<=n;j++)
        {
            prevDiff=max(prevDiff,profit[i-1][j-1]-price[j-1]);       // prevDiff is max(profit[t-1][j] – price[j]) for all j in range [0, i-2]
            profit[i][j]=max(profit[i-1][j],prevDiff + price[j]);
        }
    }
