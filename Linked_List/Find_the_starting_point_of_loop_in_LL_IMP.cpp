...
3. The point at which they meet is the start of the loop.
  ...
  //SEE GFG FO BETTTER EXPLANATION  BOTH METHODS ARE IMPORTANT
...
Node* detectAndRemoveLoop(Node* head)
{
    ...
    // ahead respectively.
    slow = slow->next;               // initially slow and fast must not equal
    fast = fast->next->next;
    ...
    slow = head;
    while (slow != fast) {              //meet at starting of loop.
        slow = slow->next;
        fast = fast->next;
    }
